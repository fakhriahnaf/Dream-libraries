<li class="{{ Request::is('mahasiswas*') ? 'active' : '' }}">
    <a href="{!! route('mahasiswas.index') !!}"><i class="fa fa-edit"></i><span>Mahasiswas</span></a>
</li>

<li class="{{ Request::is('bukus*') ? 'active' : '' }}">
    <a href="{!! route('bukus.index') !!}"><i class="fa fa-edit"></i><span>Bukus</span></a>
</li>

<li class="{{ Request::is('mahasiswas*') ? 'active' : '' }}">
    <a href="{!! route('mahasiswas.index') !!}"><i class="fa fa-edit"></i><span>Mahasiswas</span></a>
</li>

<li class="{{ Request::is('visitors*') ? 'active' : '' }}">
    <a href="{!! route('visitors.index') !!}"><i class="fa fa-edit"></i><span>Visitors</span></a>
</li>

<li class="{{ Request::is('films*') ? 'active' : '' }}">
    <a href="{!! route('films.index') !!}"><i class="fa fa-edit"></i><span>Films</span></a>
</li>

<li class="{{ Request::is('merchants*') ? 'active' : '' }}">
    <a href="{!! route('merchants.index') !!}"><i class="fa fa-edit"></i><span>Merchants</span></a>
</li>

<li class="{{ Request::is('nanunas*') ? 'active' : '' }}">
    <a href="{!! route('nanunas.index') !!}"><i class="fa fa-edit"></i><span>Nanunas</span></a>
</li>

<li class="{{ Request::is('cantiks*') ? 'active' : '' }}">
    <a href="{!! route('cantiks.index') !!}"><i class="fa fa-edit"></i><span>Cantiks</span></a>
</li>

<li class="{{ Request::is('ayams*') ? 'active' : '' }}">
    <a href="{!! route('ayams.index') !!}"><i class="fa fa-edit"></i><span>Ayams</span></a>
</li>

<li class="{{ Request::is('bebeks*') ? 'active' : '' }}">
    <a href="{!! route('bebeks.index') !!}"><i class="fa fa-edit"></i><span>Bebeks</span></a>
</li>

